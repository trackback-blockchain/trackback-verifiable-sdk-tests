...
describe('Trackback Key Tests', () => {
    describe('generate', () => {
        it('default is ed25519', async () => {
            const key = await JsonWebKey2020.generate();
            // console.log(key)
            expect((key.publicKeyJwk).kty).to.equal('OKP');
            expect((key.publicKeyJwk).crv).to.equal('Ed25519');
        });
        ...
        it('supports RSA 2048 signature:PS256', async () => {
            ...
            expect((key.publicKeyJwk).kty).to.equal('RSA');
        });
        ...
        it('supports EC crv:P-384 sinature: ES384	', async () => {
            ...
            expect((key.publicKeyJwk).kty).to.equal('EC');
            expect((key.publicKeyJwk).crv).to.equal('P-384');
        });
    });
